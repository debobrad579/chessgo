...
#[cfg(test)]
mod test {
    use crate::{fen::FENError, perft::perft, state::State};
    ...
    fn perft_pos_3() -> Result<(), FENError> {
        let state = State::try_from("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1")?;
    ...
    fn perft_pos_4() -> Result<(), FENError> {
        let state =
            State::try_from("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1")?;

        assert_eq!(perft(&state, 1), 6);
        assert_eq!(perft(&state, 2), 264);
        assert_eq!(perft(&state, 3), 9_467);
    ...
    fn perft_pos_5() -> Result<(), FENError> {
        let state = State::try_from("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8")?;
    ...
    fn perft_pos_6() -> Result<(), FENError> {
        ...
        assert_eq!(perft(&state, 2), 2_079);
        assert_eq!(perft(&state, 3), 89_890);
